...
node* BinarySearchTree::IsBST(node*r)
{
    if(r !=NULL)
    {
    cout<<"root is not null so It is BST lets check 2nd condition"<<endl;
    }
     if (r->data > r->left->data && r->data < r->right->data)
    {
        
    cout<<"to check that left child is less then root && right child is greater then root"<<endl;
    }
     if(r->data > r->left->left->data && r->data < r->right->right->data)
    {
    cout<<"to check the sub childs of root "<<endl;
